...
function createProjectSymbolByName($pid) {
    ...
    return createProjectSymbol($pid,$name != '' ? $name[0] : null,$color);
...
function getProjectSymbolSource($pid) {
    ...
    return getSymbolSource($code[0],$code[1]);

}

function getSymbolSource($char, $colors) {
    $raw = mb_convert_encoding($char,'UTF-16BE','UTF-8');
    $raw = unpack('n',$raw);

    $o= str_pad(dechex($colors & 0xFF ),2,'0',STR_PAD_LEFT);
...
function setProjectSymbolFromSource($pid, $code) {
    ...
    $dec = decodeSymbol($code);
    return createProjectSymbol($pid,$dec[0],$dec[1]);
