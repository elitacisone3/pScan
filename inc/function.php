...
function set132Mode($mode) {
    global $SETTINGS;

    if (pluginCall('on132Mode',true,$mode)) return;
    ...
    if (@$SETTINGS['colors']) {
        ...
        if (getConfig('tty.lineWrap',true)) echo "\033[7h";
...
function dispatch() {
    ...
    if (isset($SETTINGS['windows']) and !$SETTINGS['windows']) echo "\n";
    ...
    if (@$SETTINGS['colors'] and getConfig('tty.lineWrap',true)) {
        echo "\033[7l";
    }

    $x = pluginCall('onBeforeDispatch',false,$text);
    if ($x) $text = $x;
...
function tagParser($text, $tagOpen='<{', $tagClose='}>') {

    $tagOpenLen=strlen($tagOpen);
    $tagCloseLen=strlen($tagClose);
    $o=array();
    $textLen = strlen($text);

    for ($i=0;$i<$textLen;$i++) {
        $p = strpos($text,$tagOpen);
        if ($p!==false) {
            $o[] = array(0,substr($text,0,$p));
            $text = substr($text,$p+$tagOpenLen);
            $f = strpos($text,$tagClose);
            if ($f!==false) {
                $t = substr($text,0,$f);
                $text=substr($text,$f+$tagCloseLen);
                $o[] = array(1,$t);
            }
        } else break;
    }

    if (strlen($text)>0) $o[] = array(0,$text);
    return $o;
