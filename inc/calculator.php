...
function parseInterval(&$dayArray, &$cox, $interval) {
    ...
    $useExtMap = isset($SETTINGS['fullNames']);
    ...
    if ($useExtMap) {
        $pid = isset($interval['pid']) ? $interval['pid'] : 0;
        for ($tcr = $tcrStart; $tcr<$tcrEnd;$tcr++) {
            if (isset($dayArray[$dayId]['pidMapX'][$pid])) {
                $dayArray[$dayId]['pidMapX'][$pid]++;
            } else {
                $dayArray[$dayId]['pidMapX'][$pid]=1;
            }
        }
    }
...
function getAllData() {
    ...
    if (isset($SETTINGS['projList'])) {
        ...
        $data = getIntervalsFrom($SETTINGS['project'],[]);
    ...
    if (isPluginMethod('parseInterval')) {

        foreach ($data['data'] as &$interval) {
            pluginFixArray($interval,'parseInterval',false,$interval,$cox);
        }
        unset($interval);
        ...
        if ($h > 0) {
            ...
            if ($doText) {
                ...
                if ($byHeat) {
                    ...
                    $str.= ttyS(isset($heat[2]) ? $heat[2] : 12,0,$heat[1]);
...
function getByMonthData($data) {
    ...
    foreach ($data['days'] as $day) {
        ...
        if ($isMap and isset($day['pidMapX'])) {

            if (!isset($month[$monthId]['pidMapX'])) $month[$monthId]['pidMapX'] = [];

            foreach ($day['pidMapX'] as $k => $v) {
               if (!isset($month[$monthId]['pidMapX'][$k])) $month[$monthId]['pidMapX'][$k] = 0;
                $month[$monthId]['pidMapX'][$k]+=$v;
            }
        ...
        // non usato: $month[$monthId]['fest'][$day['day']] = $day['fest'];
